...
router.get("/papers/:abstract", async function (req, res) {
  const data = await selectRows(
    `SELECT * FROM papers WHERE abstract ILIKE $1 LIMIT 20000`,
    [req.params.abstract]
  );
  if (data.length === 0) {
    res.status(404).json({ message: "not found" });
...
router.get(
  "/papers/:abstract/:abstract2/:abstract3/:startYear/:endYear",
...
router.get(
  "/authors/:name/:name2/:name3/:name4/:name5",
  async function (req, res) {
    const data = await selectRows(
      `SELECT * FROM authors WHERE name ILIKE $1 AND name ILIKE $2 
...
router.get("/keywords/:keyword/:startYear/:endYear", async (req, res) => {
  const data = await selectRows(
    `SELECT * FROM keywords WHERE keyword = $1 AND (year BETWEEN $2 and $3)`,
